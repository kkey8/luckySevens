...
var array = [];
	...
	if(startingBet <= 0 || isNaN(currentMoney)) {
		...
		do {
			...
			var indexMaxWon = array.indexOf(Math.max(...array)) + 1;
			var totalRolls = array.length;
			var description = ["Starting Bet", "Total Rolls Before Going Broke", "Highest Amount Held/Won", "Roll Count at Highest Amount Won"];
			var myData = ["$ " + startingBet, totalRolls+" roll(s)", "$ "+ maxWon, indexMaxWon + " roll(s)"];
